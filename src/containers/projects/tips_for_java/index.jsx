  ...
  return (
    <>
      ...
      <div>
        ...
        <p>Definition from wikipedia: Thread safety is a computer programming concept applicable to multi-threaded code.
          Thread-safe code only manipulates shared data structures in a manner that ensures that all threads behave
          properly and fulfill their design specifications without unintended interaction. There are various strategies
          for making thread-safe data structures.</p>
        ...
        <p>in the first time call to run(), the code will print the message, but for the second time, it will not. For
          the case that run() is called in parallel by multiple threads, assume the first thread has executed just
          passed the sout and before change value of boolean, the second thread comes in and go through the
          if-statement. The message will be printed twice, which makes the code thread unsafe.</p>
      ...
      <div>
        ...
        <p>A string statement will initiate new object and unchangeable object, while the later two will only operate on
          exiting objects. Thus string is not recommanded when lots of modification string content exists.</p>
        <p>As StringBuffer is thread safe and StringBuilder is thread unsafe, thus StringBuffer is pro multi thread
          environment and StringBuilder is pro single thread environment.</p>
      ...
      <div>
        ...
        <ul>
          <li><code>indexOf(int ch)</code> return the index of the first occurrence of specific character, -1 if not
            occurred
          </li>
          <li><code>indexOf(int ch, int fromIndex)</code> return the first occurrence of specific character after given
            index position
          </li>
      ...
      <div>
        ...
        <p>Only abstract class can have abstract methods, however, abstract method is not mandatory for abstract
          class.</p>
        <p>Abstract class cannot be instantiated.</p>
        <p>Abstract class cannot be "final"-ed. // referes to 2, final class cannot be extended, while abstract class
          must have normal classes to extend</p>
      </div>
